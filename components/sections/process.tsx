    ...
    <section id="process" className="bg-[#091321] py-24 text-white sm:py-28 lg:py-32">
      <div className="mx-auto max-w-[1180px] px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {processSteps.map((step, index) => <Reveal key={step.number} delay={index * .035}><article className="relative min-h-[225px] rounded-2xl border border-white/10 bg-[#0d1929] p-6"><span className="font-mono text-xs font-semibold text-blue-400">{step.number}</span><h3 className="mt-10 text-lg font-semibold">{step.title}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{step.text}</p></article></Reveal>)}
        </div>
      </div>
    </section>
  );
